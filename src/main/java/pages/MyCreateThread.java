...
public class MyCreateThread
{
	...
	@RequestMapping("/create_thread.cgi")
	...
	)
	{
		...
		if(-1 < global_mp.getImage_name().indexOf("no_image"))
		{
			model.addAttribute("err", "Ошибка. Тред без картинки создавать нельзя!");
		...
		else
		{
			...
			MyBaseController.createNewThread( global_mp );
			return "redirect:/show_board.cgi?page=0";
		}
	}
